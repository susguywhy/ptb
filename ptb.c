...
*  v1.0 - Initial Release - Originally written for Uno R4 WiFi but ported over to
   ...
   v1.2 - Simplification  - Removed 1 Day timer and combined into 10 sec timer.
*
...
void push_charge_hvtb(void) {
  ...
  Serial.print("charge_hvtb_ts = timeClient.getEpochTime() => ");
...
void cb_10sec_periodic() {
  ...
  //update RTC; don't worry about missing an update
  timeClient.update();
  ...
  if(temp_elapsed >= SECONDS_IN_A_DAY) {
    push_charge_hvtb();
...
void calculateUptimeAndPost(String prepend) {  
  sprintf(Uptime_Str, "%lu sec. [TE/CHTS]: %lu / %lu", (timeClient.getEpochTime() - ts_startup_time), temp_elapsed, charge_hvtb_ts);
 ...
 */
  ...
  while(timeClient.update() == false);  //wait a little for things to "catch up"
  ...
  uptime_cloud = "[PTB v.1.2] PB: " + String(btn_press_cnt) + " NWI: " + String(wloss_cnt) + " Epoch: " + String(ts_startup_time) + "\n";  
